...
def plot_samples(autoencoder, device):
    ...
    for i, x in enumerate(np.linspace(-1, 1, NUM_IMAGES)):
        for j, y in enumerate(np.linspace(-1, 1, NUM_IMAGES)):
            ...
            reconstruction = torch.reshape(
                reconstruction, shape=torch.Size([28, 28]))
...
def generate_gif(autoencoder, dataloader_params, dataloader_func, device):
    NUM_SAMPLES = 30
    SAVE_IMAGE_W = 10
    fill_image = np.zeros((NUM_SAMPLES, SAVE_IMAGE_W*28, SAVE_IMAGE_W*28))

    dataloader_params["hyperparams"]["batch_size"] = SAVE_IMAGE_W**2
    train_iter = iter(dataloader_func(**dataloader_params["hyperparams"])[0])
    x_1, x_2 = (next(train_iter)[0]).to(
        device), (next(train_iter)[0]).to(device)
    z_1, _, _, _ = autoencoder(x_1)
    z_2, _, _, _ = autoencoder(x_2)

    zs = torch.stack(
        [z_1 + (z_2 - z_1)*t for t in torch.linspace(0, 1, NUM_SAMPLES)])

    for i in range(NUM_SAMPLES):
        batch_decoded = autoencoder.decoder(zs[i]).reshape(
            SAVE_IMAGE_W, SAVE_IMAGE_W, 28, 28).detach().cpu().numpy()

        for j in range(SAVE_IMAGE_W):
            for k in range(SAVE_IMAGE_W):
                fill_image[i, j*28:(j+1)*28, k*28:(k+1) *
                           28] = batch_decoded[j, k, :, :]
...
def visualize(fp, architecture_params, dataloader_params, dataloader_func, resume):
    ...
    # Interpolation
    generate_gif(autoencoder, dataloader_params, dataloader_func, device)
